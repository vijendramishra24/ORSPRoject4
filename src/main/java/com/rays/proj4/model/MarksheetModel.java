...
public class MarksheetModel {
    ...
    public long add(MarksheetBean bean) throws ApplicationException,
       ...
       StudentModel sModel = new StudentModel();
        ...
        //System.out.println(bean.getRollNo()+bean.getStudentId()+bean.getName()+bean.getPhysics());
...
System.out.println(pk+"  "+bean.getRollNo()+"  "+bean.getStudentId()+"  "+bean.getName()+"  "+bean.getPhysics()+"  "+
            bean.getChemistry()+"  "+bean.getMaths()
+"  "+bean.getCreatedBy()+"  "+bean.getModifiedBy()+"  "+ bean.getCreatedDatetime()+"  "+bean.getModifiedDatetime());
    ...
    public List getMeritList(int pageNo, int pageSize)
        ...
        ArrayList list = new ArrayList();
        StringBuffer sql = new StringBuffer(
                "SELECT `ID`,`ROLL_NO`, `NAME`, `PHYSICS`, `CHEMISTRY`, `MATHS` , (PHYSICS + CHEMISTRY + MATHS) as total from `ST_MARKSHEET` WHERE(PHYSICS>70 AND CHEMISTRY>70 AND MATHS>70) order by total desc");
        // if page size is greater than zero then apply pagination
        if (pageSize > 0) {
            // Calculate start record index
            pageNo = (pageNo - 1) * pageSize;
